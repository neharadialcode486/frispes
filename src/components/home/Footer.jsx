...
const Footer = () => {
    return (
        <footer className='bg-off-blue sm:py-14 py-8 relative z-10 -mt-1'>
                            ...
                            <div key={index} className="sm:w-fit mt-6 lg:mt-0 w-1/2">
                                ...
                                <div className="flex flex-col gap-[9px] mt-3 sm:mt-6">
                                    {items.list.map((obj, idx) => (
                                        <a key={idx} target={idx === items.list.length - 2 ? "_blank" : undefined} rel={idx === items.list.length - 2 ? "noreferrer" : undefined} href={obj.URL} className='w-fit font-lato font-normal text-white text-base transition-slow hover:text-off-orange leading-custom-xl'>{obj.subTitle}</a>
                                    ))}
                                </div>
                            </div>
                        ))}
                        <div className="sm:max-w-[212px] sm:w-full mt-6 lg:mt-0 w-1/2">
                            ...
                            <div className="flex flex-col gap-[9px] mt-3 sm:mt-6">
                                {FOOTER_CONTACT_DATA_LIST.map((obj, idx) => (
                                    <a key={idx} target={idx === 1 ? "_blank" : undefined} rel={idx === 1 ? "noreferrer" : undefined} href={obj.URL} className='w-fit font-lato font-normal text-white text-base transition-slow hover:text-off-orange leading-custom-xl flex gap-2'><span className='font-bold'>{obj.text}</span> {obj.subTitle}</a>
                                ))}
                            </div>
                        </div>
                    </div>
